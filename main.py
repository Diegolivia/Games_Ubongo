...
class Dice:
    ...
    #Tira para las caras del 2 al 7
    def TirarDice(self):
        g = rand.randint(2, 7)
...
def on_mouse_press(x, y, button, modifiers):
    if(button==mouse.LEFT):
        if(engine.GameState==0):
            if((x>=530 and x<=1070) and (y>=330 and y<=390)):
                engine.iniciarGame()
            elif ((x>=530 and x<=1070) and (y>=230 and y<=290)):
                engine.continuarGame()
            elif ((x>=530 and x<=1070) and (y>=130 and y<=190)):
                pyglet.app.exit()
                window.close()
